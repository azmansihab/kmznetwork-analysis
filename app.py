...
max_homes = st.sidebar.slider("Maksimal Rumah per Tiang", 1, 20, 10)

col1, col2 = st.columns(2)
with col1:
    file_tiang = st.file_uploader("Upload KMZ Tiang", type=['kml', 'kmz'], key="tiang")
with col2:
    file_rumah = st.file_uploader("Upload KMZ Rumah", type=['kml', 'kmz'], key="rumah")
...
if file_tiang and file_rumah:
    ...
    if gdf_tiang is not None and gdf_rumah is not None:
        st.success(f"✅ Data Siap! Terdeteksi {len(gdf_tiang)} Tiang dan {len(gdf_rumah)} Rumah.")
        
        # Tombol Proses muncul hanya jika data valid
        if st.button("🚀 MULAI ANALISIS SEKARANG"):
            with st.spinner("Sedang memproses rute jalan... Mohon tunggu."):
                # Ambil area untuk download jalan
                ...
                for i, tiang in gdf_tiang.iterrows():
                    ...
                    # Ambil 10 terdekat
                    valid_homes = sorted(valid_homes, key=lambda x: x['dist'])[:max_homes]

                    for home in valid_homes:
                        points_for_boundary.append(home['coords'])
                        fol.newpoint(name=f"{kode_tiang}-{home['name']}", coords=[home['coords']])
                ...
                output_path = "ISP_Final_Analysis.kml"
                kml_out.save(output_path)
                st.balloons()
                
                with open(output_path, "rb") as f:
                    st.download_button("📥 DOWNLOAD HASIL KMZ", f, file_name=output_path)
    else:
        st.warning("Gagal memproses file. Pastikan format KML/KMZ benar.")
else:
    st.info("💡 Silakan unggah file Tiang dan Rumah untuk memunculkan tombol proses.")